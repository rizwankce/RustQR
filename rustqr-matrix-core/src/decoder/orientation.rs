#![allow(dead_code)] // Strict core uses only structural validation; transforms stay private helpers.
...
pub(super) fn timing_pattern_ratios(matrix: &BitMatrix) -> Option<(f32, f32)> {
    ...
    // This direct module-line sampler is deliberately local to the matrix
    // core: detector `Point` geometry has no place in an alloc-only crate.
    let horizontal: Vec<bool> = (8..(dim - 8)).map(|x| matrix.get(x, 6)).collect();
    let vertical: Vec<bool> = (8..(dim - 8)).map(|y| matrix.get(6, y)).collect();
    Some((alternation_ratio(&horizontal), alternation_ratio(&vertical)))
}
...
pub(crate) fn validate_structural_patterns(
    matrix: &BitMatrix,
