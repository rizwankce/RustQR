...
fn main() {
    ...
    match args.next().as_deref() {
        ...
        _ => {
            ...
            eprintln!(
                "       qrtool reading-rate [--profile monitor-smoke|nominal-smoke|payload-validated] [--dataset-root PATH] [--artifact PATH] [--limit N] [--max-working-dim N] [--emergency-cutoff-ms N]"
            );
            std::process::exit(2);
        }
    }
